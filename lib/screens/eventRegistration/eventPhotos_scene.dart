...
class EventPhotosScene extends StatelessWidget {
  final Store<AppState> store;

  EventPhotosScene(this.store);
  ...
  Widget buildTitle() {
    return Padding(
      ...
      child: Text(
        "We need some photos of your event",
...
class _ViewModel {
  ...
  factory _ViewModel.create(Store<AppState> store) {
    _insertPhoto(File file, int index) {
      ...
      if (event.photos.length == 0 || index > event.photos.length - 1) {
        event.photos.add(file);
      } else {
        event.photos[index] = file;
      }

      store.dispatch(UpdateEventAction(event));
      store.dispatch(ValidateEventPhotosAction());
    }

    _proceedToNextScene() {
      store.dispatch(ProceedToEventDateSceneAction());
    }

    return _ViewModel(
      photos: store.state.eventRegistrationState.event.photos,
      insertPhoto: _insertPhoto,
      fieldValidations: store.state.eventRegistrationState.fieldValidations,
      canProceedToNextScene: store.state.eventRegistrationState.sceneValidations.isValidEventPhotosScene,
